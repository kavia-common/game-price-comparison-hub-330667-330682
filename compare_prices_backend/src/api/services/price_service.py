...
Scraping strategy:
  ...
  2. If USE_MOCK_FALLBACK=true (env var) AND all scrapers return zero real (priced)
     results, fall back to the mock catalog so the UI remains functional in sandboxed
     environments (e.g., development containers without external internet access).
  3. DEBUG_SCRAPER_ERRORS=true surfaces per-store failure reasons directly in the
